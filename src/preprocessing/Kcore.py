...
def KcoreUser(data: DataFrame, K: int)-> "DataFrame":
    ...
    userK = userK.filter(userK[1] > K).select('userId').withColumnRenamed('userId','u_userId')
...
def KcoreItem(data: DataFrame, K: int)-> "DataFrame":
    ...
    itemK = itemK.filter(itemK[1] > K).select('title_new').withColumnRenamed('title_new','i_title_new')
...
def Kcore(data: DataFrame, K: int)-> "DataFrame":
    ...
    while True:
        ...
        if datau.count()  == datai.count():
            print(f"\nNumber of K_core_user and K_core_item filtering iteration: {counter}")
            return data
        else:
            if counter>100:
